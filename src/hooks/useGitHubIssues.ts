...
export function useGitHubIssues({ url }: { url: string }) {
  ...
  const [issuesGroupedByLabel, setIssuesGroupedByLabel] =
    useState<IssuesGroupedByLabel>({});
  ...
  useEffect(() => {
    ...
    if (!url.startsWith("https://github.com/")) {
      ...
      setIssuesError(
        new Error("The url must start with [https://github.com/]")
      );
      return;
    }
    ...
    if (urlParts.length !== 2) {
      ...
      setIssuesError(
        new Error(
          "The url must be in the format [https://github.com/USER/REPO]"
        )
      );
      return;
    }
...
function filterUnlabelledIssuees(arg0: { issues: Issue[] }) {
